# JD Grading Scoring Protocol (Call 2)

> Read by the automated `devin -p` JD grader Call 2 (scoring). This protocol
> covers Steps 3-4: compute score + output JSON. The reasoning (Steps 1-2)
> is produced by a separate call — see
> `_config/jd-grading-reasoning-protocol.md`.
...
- No GAPs at all → +0.3 (polish bonus for exceptional fit)
...
```json
{
  ...
  "quantitative_base": 9.0,
  "subjective_adjustment": -0.5,
  "per_criterion": [
    {"requirement": "...", "tier": "core", "assessment": "DIRECT_HIT", "comment": "specific evidence"}
  ],
...
**≥ 8** to proceed to resume customization. An 8 means the candidate's full
background matches the JD's core requirements well enough to justify
