...
2. Customize: pre-copy the base resume in _config/profile/base-resume/ to stages/2_drafts/<company-role>/[TBD] resume-v1.md, then spawn the `customizer` subagent (customizer-model). It reads the pre-copied base, _config/resume-customization-protocol.md, _config/profile/full-experience/ (reference pool), and few-shot examples. It edits the resume in place, self-measures with count_lines.py, and self-trims if over 75 lines. Pass the JD grade (from the [score] job-description.md filename) to the customizer.
