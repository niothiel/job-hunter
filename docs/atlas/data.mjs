...
<p>Code orchestrates, LLMs cognate. <code>pipeline/__main__.py</code> + the <code>pipeline/</code> package handle all plumbing (file I/O, HTTP fetching, state management, routing) via Python. A LangGraph StateGraph runs the per-job pipeline with conditional edges and an optimize cycle. LLM agents are invoked through the configured Devin, Codex, or Claude CLI for 5 cognitive tasks only: feasibility checking, JD grading, resume customization, resume grading, and truthfulness verification.</p>
